...
class Panel_admin extends CI_Controller
{
		...
		if ($sess == NULL) {
			...
			switch ($aksi) {
				...
				case 'lulus':
					...
					$acts = $this->admin->update('change-lulus-verif', $data);
					$this->session->set_flashdata('msg', 'Data berhasil diubah!');
					redirect('panel_admin/verifikasi');
					break;
			...
			$data = array(
				...
				'v_siswa'	=> $this->admin->verifikasi('siswa')->ori,
				'v_thn'		=> $thn
			);
	...
	public function set_pengumuman($aksi = '', $id = '')
	{
		...
		if ($sess == NULL) {
			...
			$data['v_siswa'] = $this->db->query("SELECT * FROM tbl_siswa inner join tbl_skema_sertifikasi on tbl_siswa.id_skema_sertifikasi = tbl_skema_sertifikasi.id_skema_sertifikasi WHERE tbl_siswa.status_verifikasi = '2' and tbl_siswa.status_pendaftaran = '1' ORDER BY tbl_siswa.id_siswa DESC ")->result();
	...
	public function data_siswa($aksi = '', $id = '')
	{
		...
		if ($sess == NULL) {
			...
			$data['asesi'] = $this->db->join('tbl_skema_sertifikasi', 'id_skema_sertifikasi', 'left')->get('tbl_siswa')->result();
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
	...
	public function detail_siswa($id)
	{
		...
		if ($sess == NULL) {
			...
			$data = array(
				'user'		=> $this->admin->base('bio', $sess),
				'judul_web'	=> "Detail Siswa"
			);

			$data['detail'] = $this->db->query("SELECT * FROM tbl_siswa left join tbl_skema_sertifikasi on tbl_siswa.id_skema_sertifikasi = tbl_skema_sertifikasi.id_skema_sertifikasi WHERE tbl_siswa.id_siswa = '$id'")->row();
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
	...
	public function update_siswa($id)
	{
		...
		if ($sess == NULL) {
			...
			$data['detail'] = $this->db->join('tbl_skema_sertifikasi', 'id_skema_sertifikasi')->get_where('tbl_siswa', $where)->row();
			$data['sertifikasi'] = $this->db->get('tbl_skema_sertifikasi')->result();
	...
	public function _rules()
	{
		...
		$this->form_validation->set_rules('pd_terakhir', 'Pendaftaran', 'required');
		...
		$this->form_validation->set_rules('kebangsaan', 'Pendaftaran', 'required');
	...
	public function kirim_pesan_aksi()
	{
		...
		$mail->Subject = 'Pesan Dari Sekolah ' . $nama_lengkap . ' LSP Lalinsa';
	...
	public function form_asessement()
	{
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			redirect('panel_admin/log_in');
			...
			$data = array(
				'user'		=> $this->admin->base('bio', $sess),
				'judul_web'	=> "Form Asessement"
			);
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/form_asessement', $data);
			$this->load->view('templates_admin/footer');
		}
	}
	...
	public function tambah_asesor_aksi()
	{
		...
		if ($sess == NULL) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$mail->Body    = $this->load->view('mail/asesor', [
				...
				if ($mail->send()) {
					...
					$this->session->set_flashdata('msg', 'Gagal kirim email!');
				}

				echo "<script>window.location.assign('" . base_url() . "/panel_admin/data_asesor')</script>";
			}
		}
	}
	...
	public function jadwalevent($aksi = '', $id = '')
	{
		...
		where tbl_siswa.status_verifikasi = 2")->result();
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/data_jadwalevent', $data);
			$this->load->view('templates_admin/footer');
		}
	}

	public function _rules_jadwalevent()
	{
		$this->form_validation->set_rules('tgl_jadwalevent', 'Tanggal', 'required');
		$this->form_validation->set_rules('nama_jadwalevent', 'Nama Event', 'required');
		$this->form_validation->set_rules('id_asesor', 'Asesor', 'required');
		$this->form_validation->set_rules('id_asesi', 'Asesi', 'required');
	}

	public function tambah_jadwalevent()
	{
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			redirect('panel_admin/log_in');
			...
			$data = array(
				'user'		=> $this->admin->base('bio', $sess),
				'asesor'	=> $this->db->query("SELECT * FROM tbl_user where level ='asesor'")->result(),
				'asesi'	=> $this->db->query("SELECT * FROM tbl_siswa")->result(),
				'judul_web'	=> "Tambah Jadwal Event"
			);
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/form_tambah_jadwalevent', $data);
			$this->load->view('templates_admin/footer');
		}
	}

	public function tambah_jadwalevent_aksi()
	{
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			redirect('panel_admin/log_in');
		} else {
			$this->_rules_jadwalevent();
			if ($this->form_validation->run() == FALSE) {
				$this->tambah_jadwalevent();
			} else {
				$nama_jadwalevent         = $this->input->post('nama_jadwalevent');
				$tgl_jadwalevent          = $this->input->post('tgl_jadwalevent');
				$id_asesor                = $this->input->post('id_asesor');
				$id_asesi         		  = $this->input->post('id_asesi');

				$data = array(
					'nama_jadwalevent' => $nama_jadwalevent,
					'tgl_jadwalevent' => $tgl_jadwalevent,
					'id_asesor' => $id_asesor,
					'id_asesi' => $id_asesi
				);

				$this->web->insert_data($data, 'tbl_jadwalevent');
				$this->session->set_flashdata('msg', 'Data Jadwal Event berhasil ditambahkan!');
				redirect('panel_admin/jadwalevent');
			}
		}
	}

	public function delete_jadwalevent($id)
	{
		$where = array('id_jadwalevent' => $id);
		$this->web->delete_data($where, 'tbl_jadwalevent');
		$this->session->set_flashdata('msg', 'Data Jadwal Event berhasil dihapus');
		redirect('panel_admin/jadwalevent');
	}

	public function update_jadwalevent($id)
	{
		$where = array('id_user' => $id);
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			...
			$data['jadwalevent'] = $this->db->query("SELECT * FROM tbl_jadwalevent WHERE id_jadwalevent = '$id'")->result();
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/form_update_jadwalevent', $data);
			$this->load->view('templates_admin/footer');
		}
	}

	public function update_jadwalevent_aksi()
	{
		$id = $this->input->post('id_jadwalevent');
		$this->_rules_jadwalevent();
		if ($this->form_validation->run() == FALSE) {
			$this->update_jadwalevent($id);
		} else {
			$nama_jadwalevent         = $this->input->post('nama_jadwalevent');
			$tgl_jadwalevent          = $this->input->post('tgl_jadwalevent');
			$id_asesor                = $this->input->post('id_asesor');
			...
			$this->web->update_data('tbl_jadwalevent', $data, $where);
			$this->session->set_flashdata('msg', 'Data Jadwal Event berhasil diubah!');
			redirect('panel_admin/jadwalevent');
		}
	}
	...
	public function tingkatan($aksi = '', $id = '')
	{
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			redirect('panel_admin/log_in');
			...
			$data = array(
				'user'		=> $this->admin->base('bio', $sess),
				'judul_web'	=> "Data Skema Sertifikasi"
			);
			$data['data'] = $this->db->query("SELECT * FROM tbl_skema_sertifikasi")->result();
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/data_skema_sertifikasi', $data);
			$this->load->view('templates_admin/footer');
		}
	}

	public function _rules_skema_sertifikasi()
	{
		$this->form_validation->set_rules('jenis_skema_sertifikasi', 'Jenis', 'required');
		$this->form_validation->set_rules('kode_skema_sertifikasi', 'kode', 'required');
		$this->form_validation->set_rules('nama_skema_sertifikasi', 'Nama Event', 'required');
	}
	...
	public function tambah_tingkatan()
	{
		...
		if ($sess == NULL) {
			...
			$data = array(
				...
				'asesor'	=> $this->db->query("SELECT * FROM tbl_user where level ='asesor'")->result(),
				'asesi'	=> $this->db->query("SELECT * FROM tbl_siswa")->result(),
				'judul_web'	=> "Tambah Skema Sertifikasi"
			);
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/form_tambah_skema_sertifikasi', $data);
			$this->load->view('templates_admin/footer');
		}
	}

	public function tambah_skema_sertifikasi_aksi()
	{
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			redirect('panel_admin/log_in');
		} else {
			$this->_rules_skema_sertifikasi();
			if ($this->form_validation->run() == FALSE) {
				$this->tambah_tingkatan();
			} else {
				$nama_skema_sertifikasi     = $this->input->post('nama_skema_sertifikasi');
				$kode_skema_sertifikasi     = $this->input->post('kode_skema_sertifikasi');
				$jenis_skema_sertifikasi    = $this->input->post('jenis_skema_sertifikasi');

				if ($_FILES["apl01"]['name'] !== "") {
					$nama1 								= str_replace("_", "", $_FILES["apl01"]['name']);
					$nama2 								= str_replace(" ", "", $nama1);
					$nama_apl01 						= time() . '-' . $nama2;
					$config['upload_path'] 				= './assets/upload/skema/';
					$config['allowed_types'] 			= 'pdf';
					$config['max_size']  				= '250000';
					$config['file_name']  				= $nama_apl01;

					$this->upload->initialize($config);

					$this->upload->do_upload('apl01');
				}

				if ($_FILES["apl02"]['name'] !== "") {
					$nama1 								= str_replace("_", "", $_FILES["apl02"]['name']);
					$nama2 								= str_replace(" ", "", $nama1);
					$nama_apl02 						= time() . '-' . $nama2;
					$config['upload_path'] 				= './assets/upload/skema/';
					$config['allowed_types'] 			= 'pdf';
					$config['max_size']  				= '250000';
					$config['file_name']  				= $nama_apl02;

					$this->upload->initialize($config);

					$this->upload->do_upload('apl02');
				}

				if ($_FILES["materi"]['name'] !== "") {
					$nama1 								= str_replace("_", "", $_FILES["materi"]['name']);
					$nama2 								= str_replace(" ", "", $nama1);
					$nama_materi 						= time() . '-' . $nama2;
					$config['upload_path'] 				= './assets/upload/skema/';
					$config['allowed_types'] 			= 'pdf';
					$config['max_size']  				= '250000';
				...
				$data = array(
					'nama_skema_sertifikasi' => $nama_skema_sertifikasi,
					'kode_skema_sertifikasi' => $kode_skema_sertifikasi,
					'jenis_skema_sertifikasi' => $jenis_skema_sertifikasi,
					'apl01' => $nama_apl01,
					'apl02' => $nama_apl02,
					'materi' => $nama_materi
				);

				$this->web->insert_data($data, 'tbl_skema_sertifikasi');
				$this->session->set_flashdata('msg', 'Data Skema Sertifikasi berhasil ditambahkan!');
				redirect('panel_admin/tingkatan');
			}
		}
	}
	...
	{
		$where = array('id_skema_sertifikasi' => $id);
		$this->web->delete_data($where, 'tbl_skema_sertifikasi');
		$this->session->set_flashdata('msg', 'Data Skema Sertifikasi berhasil dihapus');
		redirect('panel_admin/tingkatan');
	}

	public function update_skema_sertifikasi($id)
	{
		$where = array('id_user' => $id);
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			...
			$data['skema_sertifikasi'] = $this->db->query("SELECT * FROM tbl_skema_sertifikasi WHERE id_skema_sertifikasi = '$id'")->row();
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/form_update_skema_sertifikasi', $data);
			$this->load->view('templates_admin/footer');
		}
	}

	public function update_skema_sertifikasi_aksi()
	{
		$id = $this->input->post('id_skema_sertifikasi');
		$this->_rules_skema_sertifikasi();
		if ($this->form_validation->run() == FALSE) {
			$this->update_skema_sertifikasi($id);
		} else {
			$nama_skema_sertifikasi     = $this->input->post('nama_skema_sertifikasi');
			$kode_skema_sertifikasi     = $this->input->post('kode_skema_sertifikasi');
			$jenis_skema_sertifikasi    = $this->input->post('jenis_skema_sertifikasi');
			...
			if ($_FILES["apl01"]['name'] !== "") {
				...
				$this->web->update_data('tbl_skema_sertifikasi', $data, $where);
			}
			...
			$this->web->update_data('tbl_skema_sertifikasi', $data, $where);
			$this->session->set_flashdata('msg', 'Data Skema Sertifikasi berhasil diubah!');
			redirect('panel_admin/tingkatan');
		}
	}
	...
	public function detail_berkas($id)
	{
		...
		if ($sess == NULL) {
			...
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/downloadupload', $data);
			$this->load->view('templates_admin/footer');
		}
	}
	...
	public function ruang_asesi()
	{
		...
		if ($sess == NULL) {
			...
			$this->load->view('templates_admin/header', $data);
			$this->load->view('templates_admin/sidebar', $data);
			$this->load->view('admin/ruang_asesi', $data);
			$this->load->view('templates_admin/footer');
		}
	}
	...
	public function upload_soal()
	{
		...
		redirect('panel_admin/ruang_asesi');
	}
}
