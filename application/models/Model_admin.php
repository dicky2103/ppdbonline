	...
	function verifikasi($menu = '', $thn = '', $status = '')
	{
		switch ($menu) {
			case 'siswa':
				...
				$this->db->join('tbl_skema_sertifikasi','tbl_siswa.id_skema_sertifikasi = tbl_skema_sertifikasi.id_skema_sertifikasi','inner');
				$this->db->where('tbl_siswa.status_verifikasi = 1 ');
				$this->db->or_where('tbl_siswa.status_verifikasi = 2');
				$this->db->where('tbl_siswa.status_pendaftaran IS NULL');
